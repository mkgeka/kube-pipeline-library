    ...
    def execute() {
	    ...
	    script.node(server) {
		    ...
		    try {
			    ...
			    script.sh 'echo "The stage --- \${STAGE_NAME} --- has been failed on branch -- \${BRANCH_NAME} the url of the job \${RUN_TESTS_DISPLAY_URL} the url of the pipeline \${JOB_DISPLAY_URL}"'
		    }
	    }
    }
}
